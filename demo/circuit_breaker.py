...
class CircuitBreaker(object):
    ...
    def __init__(self,
        ...
        # Existing init code...
        self._half_open_attempts = 0  # Track successful attempts in half-open
    ...
    def _decorate_sync(self, function):
        ...
        def wrapper(*args, **kwargs):
            if self.state == STATE_OPEN:
                ...
                if self._allow_request_in_half_open():
                    try:
                        ...
                        if self.is_failure(type(e), e):
                            self.__call_failed()  # Transition back to open on failure
                        raise
                else:
                    if self.fallback_function:
                        return self.fallback_function(*args, **kwargs)
    ...
    def __call_succeeded(self):
        if self._state == STATE_HALF_OPEN:
            ...
            if self._half_open_attempts >= 1:  # Close after one successful half-open attempt
                self._state = STATE_CLOSED
                self._failure_count = 0
                self._last_failure = None
        else:
    ...
    def _allow_request_in_half_open(self):
        # Allow requests based on the HALF_OPEN_ALLOWANCE percentage
        return random() < self.HALF_OPEN_ALLOWANCE

    @property
    def state(self):
        if self._state == STATE_OPEN and self.open_remaining <= self._recovery_timeout // 2:
