...
class CircuitBreaker:
    ...
    def __init__(
        ...
        include: Iterable[type[ExceptionType] | Callable[[Any], bool]] | None = None,
        ...
        self._included_exceptions = list(include or [])
    ...
    def included_exceptions(
        ...
        """
        return tuple(self._included_exceptions)

    def add_included_exception(self, exception: type[ExceptionType]) -> None:
        """Add an exception to the list of excluded exceptions."""
        with self._lock:
            self._included_exceptions.append(exception)

    def add_included_exceptions(self, *exceptions: type[ExceptionType]) -> None:
        """Add exceptions to the list of excluded exceptions."""
        for exc in exceptions:
            self.add_included_exception(exc)

    def remove_included_exception(self, exception: type[ExceptionType]) -> None:
        """Remove an exception from the list of excluded exceptions."""
        with self._lock:
            self._included_exceptions.remove(exception)
    ...
    def is_system_error(self, exception: ExceptionType) -> bool:
        ...
        for inclusion in self._included_exceptions:
            if type(inclusion) is type:
                if issubclass(exception_type, inclusion):
                    return True
            elif callable(inclusion):
